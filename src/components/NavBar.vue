<template>
    ...
    <v-app-bar class="d-flex justify-space-around white"  height="125px;">
        <!-- to getback to old design all you have to do is get ride of column and rows-->
        <v-row>
          <v-col md="12">
            <v-img src="../Photos/Logo/RMA-logo-flat.jpg" height="125" width="175" v-on:click="reroute()"></v-img>
          </v-col>
        </v-row>

        <v-row>
          <v-col md="12">
          <v-btn v-for="tab in tabName" :key="tab.id" text class="font-weight-light mx-2" route :to="tab.src" exact inline-block>
            <span class="text-uppercase" text>{{tab.title}}</span>
          </v-btn>

          <v-btn v-for="tab in tabName2" :key="tab.id" text class="font-weight-light mx-2" route :to="tab.src" exact inline-block>
            <span class="text-uppercase" text>{{tab.title}}</span>
          </v-btn>
          <v-btn text class="font-weight-light mx-2" route to="/Calendar" exact inline-block>
            <span class="text-uppercase" text><v-icon>mdi-calendar</v-icon></span>
          </v-btn>
          <v-btn text class="font-weight-light mx-2" route to="/TyingVideos" exact inline-block>
            <span class="text-uppercase" text><v-icon>video_library</v-icon></span>
          </v-btn>
          <!--Account Dialog -->
          <v-menu offset-y>
              <template v-slot:activator="{ on }">
                <v-btn v-on="on" text icon tile class="font-weight-light pa-2 mx-2">
          ...
          <v-card>
              ...
              <v-card-text>
                Dont have a account? <v-btn class="font-weight-light" route to="/SignUp" text @click="dialogLogin=false; dialogAccount=false">Sign Up Here</v-btn>
              <br/>
                 Forgot Password? <v-btn class="font-weight-light" route to="/Forgot" text @click="dialogLogin=false; dialogAccount=false">Reset</v-btn>
              </v-card-text>
              <!--<v-card-text> Forgot Password? <v-btn class="font-weight-light" route to="/Forgot" text @click="dialogLogin=false; dialogAccount=false">Reset</v-btn></v-card-text>-->
...
export default
{
  ...
  methods:
  {
    ...
    login()
    {
      ...
      {
        err = "Your username or password does not match any records"
        this.error = err
